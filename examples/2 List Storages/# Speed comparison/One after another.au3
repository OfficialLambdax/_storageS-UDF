...
$hTimer = TimerInit()
For $i = 1 To $nIterations
	_storageMLi_GetElements(123, True)
Next
$nTime = TimerDiff($hTimer)
ConsoleWrite("MLi Map GetElements took: " & $nTime & " ms (" & $nTime / $nIterations & " ms/avg)" & @CRLF)
...
Next
$nTime = TimerDiff($hTimer)
ConsoleWrite("MLi Array GetElements took: " & $nTime & " ms (" & $nTime / $nIterations & " ms/avg)" & @CRLF)
...
$hTimer = TimerInit()
For $i = 1 To $nIterations
	_storageMLiRapid_AddElement(123, $i)
Next
$nTime = TimerDiff($hTimer)
ConsoleWrite("MLi Rapid Add took: " & $nTime & " ms (" & $nTime / $nIterations & " ms/avg)" & @CRLF)
_storageMLi_DestroyGroup(123)
